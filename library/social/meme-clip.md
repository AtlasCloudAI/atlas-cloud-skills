...
- 🎬 **Video** — Seedance 2 (resolve live; e.g. `bytedance/seedance-2.0/text-to-video`, or i2v `bytedance/seedance-2.0/image-to-video`) for the animated version
...
3. **(Optional) Animate it** — "Add a 3s zoom-and-shake meme motion to this image, keep the text static." → `atlas_generate_video` with **Seedance 2** i2v (`-fast` is plenty for memes).
...
Clip (Seedance 2): "Meme motion: quick punch-in zoom with a tiny camera shake on the subject, text overlay stays perfectly still, 3 seconds."
```
...
- For a pure text-to-video meme (no source still), use **Seedance 2** t2v directly and describe the gag in one line.
