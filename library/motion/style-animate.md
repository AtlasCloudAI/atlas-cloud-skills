...
- 🎬 **Video** — Seedance 2 image-to-video (resolve live; e.g. `bytedance/seedance-2.0/image-to-video`); `-fast` is cheapest for drafts
...
3. **Draft the motion** — describe the style + the movement → maps to `atlas_generate_video` with **Seedance 2 i2v** (`-fast` first).
...
4. **Final render** — re-run the keeper at full quality on **Seedance 2** or **Kling 3** for smoother, cleaner motion.
...
- Iterate the look on **Seedance 2 `-fast`** (cheapest) until the style holds, then render the final once.
